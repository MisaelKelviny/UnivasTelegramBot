...
def portalAluno(bot, update):
    bot.send_message(chat_id=update.message.chat_id,
                     text="O portal do Aluno pode ser acesso atráves do link: \n http://portalaluno.univas.edu.br/SistemaWeb/mentornet/centralaluno/PA_Login_1.php")
...
def editais(bot, update):
    bot.send_message(chat_id=update.message.chat_id, text="Os editais são disponibilizados dentro do site da univas, você pode visualizar em: https://www.univas.edu.br/maisEditais.asp")

def filtro(bot, update):
    bot.send_message(chat_id=update.message.chat_id, text="Olha o Zé!")
...
# Handlers
start_command_handler = MessageHandler((Filters.regex(re.compile(r'eae', re.IGNORECASE)) | \
...
contato_handler=MessageHandler((Filters.regex(re.compile(r'contato', re.IGNORECASE)) | Filters.regex(re.compile(r'contatos', re.IGNORECASE)) ) , contato)
...
portal_handler=MessageHandler(Filters.regex(re.compile(r'portal', re.IGNORECASE)), portalAluno)
editais_handler=MessageHandler((Filters.regex(re.compile(r'editais', re.IGNORECASE)) | (Filters.regex(re.compile(r'edital', re.IGNORECASE)))), editais)
...
dispatcher.add_handler(portal_handler)
dispatcher.add_handler(vestibular_handler)
dispatcher.add_handler(editais_handler)
